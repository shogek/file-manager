...
class IpcService implements IIPcService {
   ...
   async createFolder({ path, name }: IpcCreateFolderArgs): Promise<IpcCreateFolderResult> {
      try {
   ...
   async deleteDirent({ path }: IpcDeleteDirentArgs): Promise<IpcDeleteDirentResult> {
      try {
   ...
   private async readWorkingFolderInternal(path: string): Promise<WorkingFolder> {
      const dirents = await fileSystemService.readFolder(path)

      const osDirents = dirents.map(async (dirent): Promise<OsFile | OsFolder> => {
         const path = `${dirent.path}/${dirent.name}`

         const metadataPromise = fileSystemService.readMetadata(path)
         const childDirentsPromise = dirent.isDirectory() ? fileSystemService.readFolder(path) : null

         const [metadata, childDirents] = await Promise.all([metadataPromise, childDirentsPromise])
